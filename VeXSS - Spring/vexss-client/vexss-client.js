	...
	client
			.ping(
					...
					function(error) {
						if (error) {
							console
									.error('Elasticsearch cluster is down. Check if the ES cluster is running or consult your operations team.');
						} else {
							console
									.log('Elasticsearch cluster is up and running!');
						}
					});
	return client;
}
				...
				if (hitCounter === 1) {
					...
					if (rawJson[0]._source.webRefs.length > 1) {
						...
						for (wrc = 0; wrc < wrlength; wrc++) {
							solutionLink = getLink(
									rawJson[0]._source.webRefs[wrc].title,
									rawJson[0]._source.webRefs[wrc].link);
						...
						solutionLink = getLink(solve._source.webRefs[0].title,
								solve._source.webRefs[0].link);
					...
					for (c = 0; c < rawJson.length; c++) {
						...
						if (rawJson[c]._source.webRefs.length > 1) {
							...
							for (wrc = 0; wrc < wrlength; wrc++) {
								solutionLink = getLink(
										rawJson[c]._source.webRefs[wrc].title,
										rawJson[c]._source.webRefs[wrc].link);
								linksList.push(solutionLink);
							}

							// Display to the console doMultipleOutput
							hitsElement = hits[c];
							doMultipleOutputManyHits(output, linksList,
									hitsElement);
							...
							solutionLink = getLink(
									rawJson[c]._source.webRefs[0].title,
									rawJson[c]._source.webRefs[0].link);
							hitsElement = hits[c];
							doSingleOutputManyHits(output, solutionLink,
									hitsElement);
						}
...
function doSingleOutputOneHit(output, solve, solutionLink) {
	output.innerHTML += titleOpenTag + solutionDisp + titleCloseTag
			+ solve._source.fullSolution + titleOpenTag + webRefDisp
			+ titleCloseTag + solutionLink;
...
function doSingleOutputManyLinks(output, solve, linksList) {
	output.innerHTML += titleOpenTag + solutionDisp + titleCloseTag
			+ solve._source.fullSolution + singleLineBreak;
	var counter;
	for (counter = 0; counter < linksList.length; counter++) {
		output.innerHTML += titleOpenTag + webRefDisp + titleCloseTag
				+ linksList[counter];
	}
}
...
function doSingleOutputManyHits(output, solutionLink, hitsElement) {
	output.innerHTML += titleOpenTag + solutionDisp + titleCloseTag
			+ singleLineBreak + hitsElement + singleLineBreak + titleOpenTag
			+ webRefDisp + titleCloseTag + singleLineBreak + solutionLink;
}

function doMultipleOutputManyHits(output, linksList, hitsElement) {
	output.innerHTML += titleOpenTag + solutionDisp + titleCloseTag
			+ singleLineBreak + hitsElement + singleLineBreak;
	var counter;
	for (counter = 0; counter < linksList.length; counter++) {
		output.innerHTML += titleOpenTag + webRefDisp + titleCloseTag
				+ singleLineBreak + linksList[counter] + doubleLineBreak;
	}
}
...
function queryCacheReportHandler() {
	var reportWindow = window
			.open(
					"report",
					"_blank",
					"menubar=yes, scrollbars=yes, titlebar=yes, resizable=yes, top=100, left=100, width=800, height=600");
	...
	for (i = 0; i < queryCacheList.length; i++) {
		reportWindow.document.write('<header class="heading">' + titleOpenTag
				+ 'Topic Question:' + titleCloseTag
				+ queryCacheList[i]["question"] + queryCacheList[i]["answer"]
				+ '</header>');
	}

	reportWindow.document
			.write('<button onclick="printReport()">Print this report</button>');
	reportWindow.document
			.write('<script>function printReport(){window.print();}</script>');
